...
import { getStockCheckTrend } from '@/services/dailyStockService'
...
export async function GET(request: Request) {
  try {
    ...
    const data = await getStockCheckTrend(filters)
    return NextResponse.json(data)
  } catch (error) {
    console.error('Error in stock trend API:', error)
    return NextResponse.json(
      { error: 'Failed to fetch stock trend' },
      { status: 500 }
    )
  }
}
